...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col relative font-sans bg-dot-pattern">
      ...
      <div className="w-full text-center space-y-3 pt-12 pb-8 relative z-10">
        ...
        <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight text-foreground pb-2">
          Sirviendo Apuntes <span className="text-primary">💅</span>
        </h1>
        <p className="text-muted-foreground text-sm md:text-base max-w-xl mx-auto px-4 font-bold">
          ¡Sube las fotos de tus libros y yo extraigo el texto! Esta data está literal para decir: QUEDAAA ✨. You're my person.
        </p>
      </div>

      <main className="flex-1 w-full max-w-[1400px] mx-auto px-4 md:px-6 pb-12 flex flex-col relative z-10">
        {hasFiles ? (
          <OcrWorkspace />
        ) : (
          <OcrDropzone />
      ...
      <footer className="w-full text-center py-4 text-xs font-bold text-pink-300">
        May we meet again 💕 | Ocr web app
      </footer>
    </div>
  );
}