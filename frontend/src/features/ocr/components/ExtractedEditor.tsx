...
export const ExtractedEditor = forwardRef<ExtractedEditorHandle, ExtractedEditorProps>(
  function ExtractedEditor({ activeFile, fontSize, onChange, onCopy }, ref) {
    ...
    return (
      <section
        className="flex-1 min-w-0 flex flex-col relative bg-pink-50 border-t-2 md:border-t-0 md:border-l border-pink-200 min-h-[220px] overflow-hidden"
      ...
      >
        ...
        <div className="flex-1 min-h-0 w-full relative flex flex-col">
          <Badge className="absolute top-2 left-3 z-10 bg-white/90 text-emerald-600 border-2 border-emerald-100 shadow-sm rounded-full px-2.5 py-0.5 text-[11px] gap-1 hover:bg-white">
            <FileText className="w-3 h-3" aria-hidden /> Texto extraído
          </Badge>

          <div className="absolute top-2 right-2 z-20 flex items-center gap-1">
            {activeFile && activeFile.status === 'idle' && (
              <Badge className="bg-white/90 text-pink-500 border-2 border-pink-100 shadow-sm rounded-full px-2.5 py-0.5 text-[11px] gap-1 hover:bg-white">
            ...
            {activeFile?.status === 'error' && activeFile.errorMessage && (
              <div
                role="alert"
                className="bg-rose-100 text-rose-600 px-2.5 py-0.5 rounded-full text-[11px] font-bold border-2 border-rose-200 flex items-center gap-1 max-w-[220px]"
              >
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  ...
                  className="btn-bounce w-8 h-8 flex items-center justify-center rounded-full bg-white border-2 border-pink-200 text-pink-500 shadow-md hover:bg-pink-50 disabled:opacity-30 disabled:cursor-not-allowed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-pink-300"
                >
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  {justCopied ? (
                    <MonkeyIcon className="w-5 h-5 motion-safe:animate-bounce" />
                  ) : (
          ...
          {searchOpen && (
            <div className="absolute top-12 right-3 z-20 bg-white border-2 border-pink-200 rounded-2xl shadow-lg p-2 flex flex-col gap-2 w-[280px]">
          ...
          <textarea
            ref={textareaRef}
            id="ocr-textarea"
            className="paper-soft w-full h-full min-h-[220px] flex-1 resize-none outline-none font-medium custom-scrollbar placeholder:text-zinc-400 pl-4 pt-10 pr-4 pb-4 focus-visible:ring-2 focus-visible:ring-pink-300 focus-visible:ring-inset"
